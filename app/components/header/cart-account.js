    ...
    if (session) {
                ...
                >
                    ...
                    >
                        <path
                            ...
                            strokeWidth="2"
                            strokeLinecap="round"
                            className="stroke-[#0ABA90] md:stroke-white"
                        />
                    </svg>
                    <div
                        onMouseOver={() => MenuHandler(true)}
                        className={`pt-2 absolute top-full right-0 min-w-full ease-in-out duration-300 ${menuOpen ? "opacity-100 z-20 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
                    >
                        <div
                            onMouseOver={() => MenuHandler(true)}
                            className="justify-end text-end flex flex-col items-end gap-2 p-4 rounded-[10px] border-2 border-white backdrop-blur-[44px] cursor-auto whitespace-nowrap"
                        >
                            <Link href="/dashboard" className="block md:hidden">
                                Dashboard
                            </Link>
                            <Link href="/dashboard/cultivars" className="hidden md:block">
                                Cultivars
                            </Link>
                            <Link href="/dashboard/orders" className="hidden md:block">
                                Orders
                            </Link>
                            <Link href="/dashboard/edit-account" className="hidden md:block">
                                Edit
                            </Link>
                            <Link href="/dashboard/account" className="hidden md:block">
                                Account
                            </Link>
                            <Link href="/dashboard/eligibility" className="hidden md:block">
                                Eligibility
                            </Link>
                            <button
        ...
        return (
            <nav className="flex justify-end items-center gap-10">
                <Link href="/login" className="flex items-center gap-2 relative tracking-wider text-xl">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                        height="19"
